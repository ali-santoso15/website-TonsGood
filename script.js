        ...
        <section class="menu-category menu-category--${category.theme}">
          <header class="menu-category__head">
            ...
            >
              ...
              <span class="menu-category__arrow" aria-hidden="true"></span>
            </button>
          </header>
          <div class="menu-category__list" hidden>
            ${items}
          </div>
        </section>
      `;
...
const bindMenuAccordion = () => {
  ...
  const setCategoryState = (category, shouldOpen) => {
    const toggle = category.querySelector(".menu-category__toggle");
    const list = category.querySelector(".menu-category__list");
  ...
  menuBoard.addEventListener("click", (event) => {
    const toggle = event.target.closest(".menu-category__toggle");
    if (!toggle) return;
    ...
    const categories = menuBoard.querySelectorAll(".menu-category");

    categories.forEach((item) => setCategoryState(item, false));
    setCategoryState(category, willOpen);
  });
};
